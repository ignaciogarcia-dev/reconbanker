...
const isInternalCurrencyExchange = (description = "") =>
  /^RECIBISTE\b/i.test(String(description).trim());
...
// Tolerancia entre movement.date (inicio del ordenante) y detail.transferDoneDate
// (acreditación). Para transferencias interbancarias el delay puede pasar de 5 min
// (caso real visto: 8.6 min). 30 min cubre la mayoría sin abrir falsos positivos.
const detailTimeToleranceSeconds = 30 * 60;
...
const incomingMovements = todayMovements.filter(
  (m) =>
    (m.amount?.amount ?? 0) > 0 &&
    !isInternalCurrencyExchange(m.description),
);
...
try {
  ...
  for (let i = 0; i < incomingMovements.length; i++) {
    ...
    // Si la validación falla (delta > tolerancia o ref ausente) NO abortamos toda
    // la corrida — descartamos esta transacción y seguimos. Las ya construidas
    // siguen en `transactions` y se devuelven al final.
    let comparison;
    try {
      comparison = validateDetailMatchesMovement(movement, detail);
    } catch (validationErr) {
      debug("detail_validation_failed_skipping", {
        index: i,
        movement: summarizeMovement(movement),
        detail: summarizeDetail(detail),
        error: validationErr.message,
        partialTransactionsBuilt: transactions.length,
      });
      continue;
    }
  ...
  debug("detail_extraction_failed", {
    message: e.message,
    partialTransactionsBuilt: transactions.length,
  });
  // Si ya construimos transacciones antes del fallo, devolvemos las parciales
  // en vez de tirarlas. El error es del flujo (DOM/click/timeout), no de los
  // datos ya validados — sería peor perderlas.
