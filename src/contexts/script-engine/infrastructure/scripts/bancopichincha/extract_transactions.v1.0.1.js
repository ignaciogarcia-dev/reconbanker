...
return {
  ...
  async poll(page, context) {
    ...
    // Always re-open movements so a fresh /transactions request fires. If the SPA
    // DOM is wedged (slow/flaky network, stale client-side render) the "see movements"
    // click silently no-ops and NO /transactions request ever fires — the page sits on
    // consolidate-position forever. A hard reload re-bootstraps the SPA (exactly what a
    // human does by refreshing), so retry once behind a full reload before giving up.
    let captured = false;
