...
describe('schedulerQueries', () => {
  it('one-shot SQL selects active, unblocked, one-shot accounts via LEFT JOIN', () => {
    ...
    expect(SCRAPABLE_ONE_SHOT_ACCOUNTS_SQL).toContain('a.scrape_blocked_reason IS NULL')
    expect(SCRAPABLE_ONE_SHOT_ACCOUNTS_SQL).toContain("COALESCE(ac.session_type, 'one-shot') = 'one-shot'")
  })

  it('persistent session SQL selects active, unblocked, persistent accounts via JOIN', () => {
    ...
    expect(PERSISTENT_SESSION_CANDIDATES_SQL).toContain('a.scrape_blocked_reason IS NULL')
